...
public class LecturerRoomChangeRequestService {
    ...
    public LecturerRoomChangeRequestService(
            LecturerRepository lecturerRepository,
            StaffEmergencyRoomChangeRepository repository
    ) {
    ...
    public List<EmergencyRoomChangeScheduleResponse> getSchedules(Integer semesterId, Integer lecturerUserId) {
        ...
        return repository.findSchedulesBySemesterAndLecturerUserId(semesterId, lecturerUserId)
                .stream()
    ...
    public List<AvailableRoomResponse> findAvailableRooms(
        ...
        StaffEmergencyRoomChangeRepository.ScheduleProjection schedule =
                findAndValidateSchedule(semesterId, scheduleId, lecturerUserId);
    ...
    @Transactional(readOnly = true)
    public List<EmergencyRoomChangeResponse> getMyRequests(Integer lecturerUserId) {
        ensureLecturerProfile(lecturerUserId);

        return repository.findChangeRequestsByRequester(lecturerUserId)
                .stream()
    ...
    @Transactional
    public EmergencyRoomChangeResponse create(
            CreateLecturerRoomChangeRequest request,
            Integer lecturerUserId
    ) {
        ensureLecturerProfile(lecturerUserId);

        StaffEmergencyRoomChangeRepository.ScheduleProjection schedule =
                findAndValidateSchedule(request.getSemesterId(), request.getSectionScheduleId(), lecturerUserId);
        ...
        repository.insertPendingLecturerChange(
                schedule.getSemesterId(),
                schedule.getScheduleId(),
                window.scope(),
                window.targetDate(),
    ...
    private void ensureLecturerProfile(Integer lecturerUserId) {
        lecturerRepository.findByUserId(lecturerUserId)
    ...
    private StaffEmergencyRoomChangeRepository.ScheduleProjection findAndValidateSchedule(
        ...
        return repository.findScheduleDetailForLecturer(semesterId, scheduleId, lecturerUserId)
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Khong tim thay lich hoc cua giang vien trong hoc ky da chon."
                ));
    }
